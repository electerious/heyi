...
- Support for custom browser executable paths in `--crawler` option and `HEYI_CRAWLER` environment variable
- Paths starting with `/` (absolute), `./` or `../` (relative) are now recognized as browser executable paths
- Puppeteer now uses `executablePath` when a custom browser path is provided
